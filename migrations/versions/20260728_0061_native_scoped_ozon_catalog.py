"""Add native scope and Seller API adapter authority to Catalog snapshots."""

import sqlalchemy as sa
from alembic import op

revision = "20260728_0061"
down_revision = "20260728_0060"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    for column in columns:
        op.add_column("marketplace_catalog_snapshots", column)

    op.drop_constraint(
        "uq_marketplace_catalog_snapshot_idempotency",
        "marketplace_catalog_snapshots",
        type_="unique",
    )
    op.create_check_constraint(
        "ck_marketplace_catalog_native_authority_complete",
        "marketplace_catalog_snapshots",
        "("
    ...
    op.create_index(
        "uq_marketplace_catalog_legacy_idempotency",
        "marketplace_catalog_snapshots",
    ...
    op.create_index(
        "uq_marketplace_catalog_scoped_idempotency",
        "marketplace_catalog_snapshots",
        [
            "tenant_ref",
            "entity_ref",
    ...
    op.create_index(
        "ix_marketplace_catalog_scope_observed",
        "marketplace_catalog_snapshots",
...
def downgrade() -> None:
    ...
    for column in (
        ...
        op.drop_column("marketplace_catalog_snapshots", column)
