  ...
  return <><section className="decision-workbench" id="decision-workbench">
          ...
          <div className="causal-policy-registry">
            ...
            {causalPolicies.length ? <div className="policy-grid">{causalPolicies.map((policy) => {
              ...
              const executionPlan = causalPolicyExecutionPlans.find((item) => item.policy_id === policy.id);
              ...
              return <article className={policy.usable ? "policy-card" : "policy-card invalid"} key={policy.id}>
                ...
                {activationHandoff?.activation_eligible && !executionPlan && <div className="policy-release-form"><strong>等待因果策略执行计划</strong><p>此区域只处理 causal_policy 来源计划；目标与适配器由服务端受控编译，不在网页中自由填写。</p></div>}
