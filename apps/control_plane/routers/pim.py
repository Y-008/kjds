...
def pim_workspace(
    ...
    query: str | None = None,
    readiness: Literal["ready", "incomplete", "blocked"] | None = None,
):
    ...
    entity_scope = runtime.scope_grants.current(
        principal=principal, store_ref=store_ref, as_of=cutoff
    )
    return run(
        lambda: runtime.scoped_pim.project(
            principal=principal,
