...
class PolicyShadowService:
    ...
    def record_evaluation(
        ...
        *,
        ...
        result = self.policies.evaluate_context(policy["id"], context)
    ...
    def validate_stage_outcome(self, release_id: str, observation_count: int) -> None:
        ...
        with Session(self.engine) as session:
            evaluation_ids = list(
                session.scalars(
                    select(PolicyEvaluationRow.id).where(
                        PolicyEvaluationRow.release_id == release_id
                    )
                )
            )
    ...
    def _context(cls, value: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(value, dict) or not value:
            raise ValueError("Policy evaluation requires a structured context")
        if len(value) > 100:
            raise ValueError("Policy context exceeds the 100-field limit")
        cls._reject_sensitive(value)
        encoded = json.dumps(value, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
        if len(encoded.encode()) > 65536:
            raise ValueError("Policy context exceeds the 64 KiB limit")
        return value
