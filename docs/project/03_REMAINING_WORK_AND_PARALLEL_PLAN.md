...
| BAS-095A | G0–G5 | 正式报价数量阶梯无损合同 | 供应链+工程+合规 | 复用同一报价 Evidence、A 级复核与 `source_offers.attributes_json`；规范化保存全部数量阶梯与显式比较数量，单价必须等于适用阶梯，三家比较数量必须一致；重复数量、非正值、缺适用阶梯、单价漂移或混合比较数量全部失败关闭；不新增报价表、采购、付款或平台写入；[工程证据](evidence/20260726_BAS_095_SUPPLIER_QUOTE_AUTHORITY.md) | BAS-095、BR-069、BR-070 | DONE_ENGINEERING |
