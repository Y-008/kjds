...
| BAS-078 | G0–G4 | 唯一运行时动作授权与 readiness 绑定 | 工程+控制面 | 计划创建/读取、许可排队及 Worker claim 均调用同一 phase-aware `authorize_action()`；动作注册表声明所需 readiness；L3 缺独立批准失败关闭，L4 缺 MFA 失败关闭；真实执行缺 `demand.real_execution`、actual 权威或财务复核时在服务层阻断 | BR-037、BR-060、BAS-077 | DONE_ENGINEERING |
