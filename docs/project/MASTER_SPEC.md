...
| BR-070 | 供应商报价独立权威门 | 公开展示价、聊天与上传文件先进入 B 级线索；只有非上传者逐项确认供应商、冻结规格、金额币种 MOQ、有效期和交付条件后，才形成 A 级复核凭证。公开展示价永不晋升。确认报价含数量阶梯时，全部阶梯必须在同一不可变原件条款中按唯一、严格递增的最小数量和正 Decimal 单价保存，并显式冻结本次比较数量；`SupplierOffer.unit_price` 必须由该数量适用的最高起订阶梯确定，不能由客户端另选或截取一档。三份不同供应商的当前已接受原件只有在比较数量一致时才允许生成 SupplierOffer 与 CM3；报价在付款前仍为 estimate | P0 |
...
| BR-082 | 批量市场扫描、观察成本 Pilot 与内容工厂 | KJDS 必须在 BR-081 上提供一个小接口、深实现的批量经营模块族：`BatchMarketScanner` 只从带来源、时间、Evidence、置信度和明确价格语义的 Ozon/1688 观察读取需求、竞品、价格带、评价/销量代理、促销、季节、缺货、产业带、供应商密度、精确变体、MOQ、交期、包装重量体积和集运距离；缺失销量不得推算成销量。漏斗必须分别报告 `exact_identity_matched`、`checkout_cost_eligible` 与 `fully_costed_candidates`，不得因下单页或成本 Evidence 缺失而把已连接的同一精确身份误报为未匹配。`OpportunityScorer` 只对服务端精确身份匹配且可复核的 `observed_checkout_price` 计算十五项 baseline/downside 筛选 CM3、现金占用、周转和风险，观察成本不得晋升 Supplier Offer、actual 或正式 CM3。`StrategyClassifier` 只能输出淘汰、探索、受控铺货、精品精细化、套装、配件、真实属性变体或店群复制建议，70%/20%/10% 仅为资源政策目标；`VariantExpansionPlanner` 只有在真实父 SKU、真实属性及 24h/72h/7d 回读满足时才能提出裂变，禁止虚假属性、重复商品和类目污染。状态固定为 `observe→match→evaluate→content_ready→pilot→scale|stop→reconcile`，复用 OperatingTask/OperationsQueue、Owner、Evidence、预算和稳定 fingerprint。内容工厂只生成可追溯俄语文案/属性/卖点/详情结构和媒体 Brief；未取得权利、Passport 或媒体 QA 时 `content_ready=false`。批量筛选输出目标只能从 50/100/200/500/1000 中选择，并冻结版本化 Profile、有限覆盖项、入选/淘汰原因及稳定排序；达到目标只表示候选短名单。Operator 可将短名单幂等创建为 KJDS 原生 PIM 的 `candidate` Product，内部 SKU 从候选 fingerprint 稳定派生且事件必须回链 run/candidate/Evidence；该动作不得调用第三方 ERP、创建报价/库存/采购/Listing 或写 Ozon。任何候选只有 downside CM3>0、数据时效/身份/规格/可采购性/媒体权利/Passport、独立批准、一次性 Permit、Readback 与止损全部满足时才可进入 1–3 件小流量 Pilot；本模块不创建 Permit、不写 Ozon、不承诺盈利。首发采购模式固定为 `sale_triggered_jit`：真实 Ozon 出单前不得创建购物车、供应商订单或付款；只有有效 Evidence 支撑、解析到同一 Product/SKU、`store_ref` 与授权店铺一致且状态为 `awaiting_packaging` 的正式 `ozon_order` FactRecord，结合重新核验的当前库存、checkout 成本和悲观 CM3，才可形成内部 `eligible_for_procurement_review`，仍不得自动采购或付款。状态未知、取消、退货、跨店、坏 Evidence 或手工 Order 均不能触发。Web 必须以真实 HTML/SVG 显示全国供应分布、市场价格带、候选漏斗、策略、CM3/现金/风险、变体树、采购触发状态和 24h/72h/7d 回读，并报告实际观察数、精确身份匹配数、checkout 成本候选数、完整成本数、downside 正数、内容就绪数和 Pilot 数，缺数据明确 `no_data/blocked`。 | P0 |
| BR-083 | KJDS 原生 Item 主档与可选 ERP 出口 | 服务端只能从授权店铺的不可变 Batch Opportunity run/candidate 重新读取精确身份、十五项成本 Evidence、悲观 CM3 与守恒结果；不接受客户端自报利润。候选先幂等进入 KJDS 原生 Product/PIM，KJDS 是商品、Evidence、利润、状态与审批唯一真源。ERPNext 及任何第三方 ERP 只是默认关闭的可选出口，不是运行依赖、主档或利润权威；启用后也只能由既有 KJDS Product 派生 `Item` 草稿和幂等 outbox，初始库存为 0，并以最小权限写后回读稳定外部 ID、`item_code`、`docstatus=0` 与 KJDS Product 引用。未配置连接器、坏 Evidence、跨店、同键异 payload 或回读差异全部失败关闭。该接口不创建采购单、收货、库存调整、付款、广告、Permit 或 Ozon 写入；真实出单后的采购仍遵守 BR-082 `sale_triggered_jit`。外部 ERP 仅用于兼容客户明确选择的系统；竞品产品只作功能与体验对标。详见 ADR-0033。 | P0 |
...
| BR-146 | 历史需求追溯与自动化控制结构合同 | 唯一 `RequirementsTraceabilityProgram.project()` 必须把历次用户建议编译为确定性、只读的机器追溯矩阵，每项强制包含需求来源、Requirement ID、机器合同、实现路径、当前版本、Owner/替补、Gate、Evidence、状态和未完成项；状态只允许 `ADOPTED_ENGINEERING/ISOLATED_IMPLEMENTED/CONTRACT_ONLY/PILOT_PENDING/BLOCKED_EVIDENCE/REJECTED_DUPLICATE`。注册表和投影只能证明结构与引用完整，所有条目的 `business_truth_status` 必须保持 `UNKNOWN` 且 `business_truth_proven=false`；禁止静态生成真人到岗、真实 RFQ、客户价值、Actual Cash CM3、生产 Gate 或 Top1 结论。隔离实现必须绑定分支与复核 HEAD、全部路径标记为 isolated、`mainline_integration_status=NOT_STARTED` 并携带选择性集成 Gate，禁止把隔离代码冒充主线或执行盲 merge/rebase。Automation Grant、Safety Case、Process Conformance、Value Ledger 与 Capability Passport 首切片只冻结合同、Owner、字段、生命周期和 Gate，`runtime_connected/creates_authority/external_write_allowed` 全为 false；不得建立第二 ERP、任务总线、事实账、审批或 Permit 权威。构造阶段一次性校验注册表，零参数 `project()` 只返回 defensive copy，不接 runtime/router/API/OpenAPI/Web/DB/migration/G-1。详见 [ADR-0096](../adr/ADR-0096-requirements-traceability-and-automation-control.md) 与 [BAS-218 Evidence](evidence/20260809_BAS_218_REQUIREMENTS_TRACEABILITY_AUTOMATION_CONTROL.md)。 | P0 |
| BR-147 | 自动经营核心选择性主线集成与分阶段追溯 | `AutomatedCommerceLoop` 必须作为只读深模块复用现有 Product、ListingDraft、SupplierOffer、ProfitScenario、Scoped Marketplace Catalog、Scoped Sourcing Intelligence 与 AI Listing；不得新建第二 ERP、报价账、任务总线或发布状态机。任何读取先通过 exact tenant/entity/store/current authority；目录与货源投影必须验证 contract、scope、as-of、snapshot、分页、计数守恒和只读控制信封，漂移时失败关闭且不得泄露上游异常值。利润只有有限 Decimal、成本 Evidence 完整且 CM3 可复核时才可推荐；公开展示价、聊天 acknowledgement/clarification/alternative/platform notice、`latest_reply_unknown` 和无法无损表达的多阶梯报价不得晋升 `SupplierOffer`、正式报价或利润事实。门店总开关、playbook 开关和 `requested_mode/effective_mode/grant_ready/runtime_execution_enabled` 四态保持默认关闭；BAS-219A 只接入核心模块、Profile 合同与 RFQ 只读回链，不接 runtime/router/API/OpenAPI/Web/DB/migration/G-1 或外写。隔离分支中的运行时/API/Web 保持独立 `ISOLATED_IMPLEMENTED` 条目，只有 BAS-186 释放共享 seam 后才能作为 BAS-219B 选择性接入。真实 SKU、六家 RFQ、Ozon 回读、28 天实证、合规结论和 Actual Cash CM3 全部继续 `BLOCKED_EVIDENCE`。详见 [ADR-0097](../adr/ADR-0097-automated-commerce-loop-and-source-linkback.md)、[自动经营 PRD](19_AUTOMATED_COMMERCE_AND_RECURSIVE_STORE_MINING_PRD.md) 与 [BAS-207/219A Evidence](evidence/20260809_BAS_207_AUTOMATED_COMMERCE_LINKBACK.md)。 | P0 |
| BR-148 | 企业定位与画像驱动测试角色编制 | 唯一只读 `EnterprisePositioningAdvisor.position(profile)` 必须从版本化企业画像、既有 18 核心岗位、12 AI 专家和 5 独立控制角色编译企业定位、价值楔子、35 个唯一测试角色、启用/待命建议、最低真人责任节点、职责分离和国家/平台缺岗；不得复制或替代 Team Control、Global Expert Team、Enterprise AI ERP、身份、任务、审批或 Permit 权威。当前 KJDS 必须定位为“企业级架构标准下的 AI 原生、证据优先、受控自动化跨境电商经营控制平面（真实现金闭环验证期）”，目标企业 AI ERP 定位在 Truth SKU Cash Verified、商业 C0、真人责任绑定、职责冲突清零和当前 G-1 通过前保持 `BLOCKED_EVIDENCE`。系统可以推荐角色，不能创建测试身份、任命真人、绑定身份、启动工作、授予生产权限或外写；所有测试角色固定 `test_shadow`、`human_binding_status=UNKNOWN`、零生产/事实晋升/外写权限。企业画像变化必须确定性重算角色并对未支持国家/平台显式报告缺岗，不得用通用角色冒充本地专业权威。 | P0 |
| BR-149 | 董事会定位冻结、八维角色编制 v2 与 90 天双线总控 | KJDS 当前产品必须冻结为“面向跨境电商经营者的、证据优先、真实现金导向、受控自动化的经营控制面”，商业顺序固定为自营验证→固定范围现金诊断→设计伙伴→SaaS/实施→企业控制平台；Venture Federation、World Model、Synthetic Economy、新国家、新平台和长期 Agent 扩编冻结 90 天。执行容量固定 80% Truth SKU/Cash/C0、20% 企业角色系统，同时最多一条经营资料主线和一条角色工程主线。`EnterprisePositioningAdvisor.position(profile)` 继续作为唯一深 Module Interface，但 v2 必须把 35 项定义为无身份语义的能力模板，使用稳定 `role_template_ref` 和 `required_now/supporting_ai/on_demand/standby` 推荐状态；`unsupported_gap` 只用于未支持国家/平台缺岗。`business_model/stage/headcount_band/markets/platforms/risk_class/primary_objective` 必须分别驱动角色、席位、容量、控制强度或下一激活角色，`enterprise_ref` 只绑定确定性企业作用域和快照，不授予任何权威。输出必须包含最多四个真人责任席位、不可兼任规则、最低真人责任、缺岗、下一角色及所需 Gate；所有绑定继续 `UNKNOWN`，不得创建 Principal、Agent、任务、Appointment、Approval、Permit、Fact、预算或外写。认证只读 `GET /v1/enterprise-positioning/current` 与计算型 `POST /v1/enterprise-positioning/recommend` 只调用该 Interface；POST 不持久化且不受停写开关阻断，但仍需读角色。现有 `/team-control` 只显示服务端顺序/状态并提供明确“未保存”的画像模拟器，不得新建第二老板页或客户端计算角色/Gate。当前目标企业 AI ERP 晋级继续要求一个 Truth SKU `CASH_VERIFIED`、商业 C0、真人责任绑定、SoD 清零和当前 G-1；私密 `real-sku-startup` 包仅 `g0-ozon-api-identities.csv` 为 `ready_for_human_review`，其余七区保持 `awaiting_inputs`，且 `automatic_import=false`、`formal_fact_promoted=false`，结构复核不等于业务 ready、生产准入或 Fact 晋升。详见 [ADR-0098](../adr/ADR-0098-board-reset-and-enterprise-role-composition-v2.md) 与 [90 天运行计划](22_BOARD_STRATEGY_AND_90_DAY_OPERATING_PLAN.md)。 | P0 |

BR-148 已由 BR-149 明确取代，仅保留历史追溯；其中“测试角色”、`test_shadow` 和任何 `test_principal_ref` 语义均不得进入当前实现、API、页面或新任务。当前唯一有效角色合同为 BR-149 的无身份能力模板 v2。
