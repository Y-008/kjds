...
| BR-144 | 1688 同页全变体与供应商公开字段采集 | 现有 `BrowserCaptureInbox` 的 1688 Provider 必须在用户显式点击的当前商品或店铺页内，优先解析同页 SSR `window.context` 的序列化结构化数据并回退可见 DOM；商品详情页在 500 行硬上限内必须把序列化矩阵中的全部公开 SKU 逐一保存为 exact `offer_id + sku_id + spec_id/variant_key` Observation item，任何行缺稳定身份或价格均整次失败，不得静默漏行或伪称全量；超过 500 行须失败并升级/拆分合同。每行保留价格、库存/已售信号、MOQ、货号/属性、重量、类目、图片以及供应商公司名、登录名、公开服务评分/复购率/评价信号。服务端必须复验 discovered=captured=item count、truncated=false，并按商品分组重算 min/max 与最低价对应变体，禁止把三件套等低价引流变体冒充六件套或目标 BOM 成本。店铺页只保存当前页最多 50 个公开商品卡并显式记录总发现量、截断与 `variant_selection_unverified`，不得伪称全店/全站完成。该能力复用 `kjds-browser-capture-envelope`、既有 C 级 Evidence、quarantine/独立绑定和 Market Recon 守恒，不新增 Supplier/Quote/Product 真源；扩展继续仅有 `activeTab+scripting+storage(session)`，不得读取或传输 Cookie、localStorage、买家身份、内部 API 响应、验证码或广域 host。所有公开价格、库存、销量和商家评分均为 `public_discovery_signal`，不得晋级 Supplier Offer、actual cost、正式销量、利润或外部写。详见 [ADR-0059](../adr/ADR-0059-browser-capture-inbox.md)。 | P0 |
| BR-145 | 1688 同款/同类候选发现、可比性与 ERP 暂存守恒 | 同一扩展必须提供用户显式触发的 1688 关键词同类搜索入口，并在当前搜索/店铺结果页把最多 50 个卡片保存为 offer-only 候选；候选在进入详情页补齐 `sku_id/spec_id` 前固定为 `requires_detail_enrichment`，不得参与精确成本或利润计算。详情页每个 SKU 必须形成唯一 ERP 暂存行并冻结 supplier、offer、sku、spec、variant、unit price、完整公开阶梯价、公开指标、来源 URL、采集时间、item hash 与 comparison key；服务端拒绝商家漂移、URL/offer 漂移、重复键、覆盖量不守恒或候选伪称已选变体。Offer 级价格表在同一起订量存在多个 SKU/BOM 价格时必须按该 SKU 自身行价隔离，无法唯一绑定则该 SKU 的阶梯价保持未知，禁止把一条低价复制到全部 SKU。只有类目、件数/BOM、尺寸、材质、交易单位等已提取比较维度完全相同的行才进入同一可比价组；维度缺失或不同仅并列展示并要求补证。跨 offer 比价只使用每个 supplier/offer 最新且 Evidence/adapter 完整的详情快照，并显式冻结参考采购量、MOQ、库存状态和价格口径；对指定参考量只可采用 `minimum_quantity <= reference_quantity` 的最高起订量阶梯，未观察到适用阶梯时不得退猜价格。默认参考量 1 时，MOQ 未知、MOQ>1、缺货、无适用公开数量价或非公开单价行不得进入最低价排行，历史快照不得伪装成第二家供应商。ERP 暂存和比价均不是 Canonical Product、Supplier Offer、actual cost、正式销量、含税/含运利润或外部平台写入。 | P0 |
