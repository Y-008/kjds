...
| BR-145 | 1688 同款/同类候选发现、可比性与 ERP 暂存守恒 | 同一扩展必须提供用户显式触发的 1688 关键词同类搜索入口，并在当前搜索/店铺结果页把最多 50 个卡片保存为 offer-only 候选；候选在进入详情页补齐 `sku_id/spec_id` 前固定为 `requires_detail_enrichment`，不得参与精确成本或利润计算。详情页每个 SKU 必须形成唯一 ERP 暂存行并冻结 supplier、offer、sku、spec、variant、unit price、公开指标、来源 URL、采集时间、item hash 与 comparison key；服务端拒绝商家漂移、URL/offer 漂移、重复键、覆盖量不守恒或候选伪称已选变体。只有类目、件数/BOM、尺寸、材质、交易单位等已提取比较维度完全相同的行才进入同一可比价组；维度缺失或不同仅并列展示并要求补证。跨 offer 比价只使用每个 supplier/offer 最新且 Evidence/adapter 完整的详情快照，并显式冻结参考采购量、MOQ、库存状态和价格口径；默认参考量 1 时，MOQ 未知、MOQ>1、缺货或非公开单价行不得进入最低价排行，历史快照不得伪装成第二家供应商。ERP 暂存和比价均不是 Canonical Product、Supplier Offer、actual cost、正式销量、含税/含运利润或外部平台写入。 | P0 |
