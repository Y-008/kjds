...
| BR-067 | Ozon 销售单触发的采购与动态选仓 | 商品候选、商品卡、Listing 草稿和已发布 Listing 均不得被称为采购订单，也不得触发供应商下单。只有 KJDS 已记录的真实 Ozon 销售订单才能建立唯一的订单履约需求；建立时不得预存国内收货地址。运营人员必须针对该销售单从跨境巴士读取当时可用的 Ozon 渠道和仓库服务，固化承运商、服务代码、国内仓 ID/名称/完整地址、地址有效时间及 Evidence 后，才可申请与同一 Product、三报价中选定 Offer、完整正 CM3 场景和采购数量绑定的采购审批。`GOOL` 输入统一规范为 `GUOO`；`UNI` 自 2026-02-10 起不得作为新接入默认渠道，只有具备当时有效证据的既有连接可标记 `legacy_only`。供应商订单确认必须由独立批准支持，且收货仓必须与已固化路线逐字段一致；随后按国内发货、仓库签收、打包贴标、国际交接追加不可变 Evidence。该合同不调用外部下单或支付，不允许 Listing 上线自动采购，也不让跨境巴士、物流商或编排器拥有订单、地址、审批和利润真源。 | P0 |
