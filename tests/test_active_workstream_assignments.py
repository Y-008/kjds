...
def _bas215c_current_task():
    return {
        "task_id": "BAS-215C",
        "state": "in_progress",
        "owner_thread_id": "019fd4c1-60c9-79a0-9338-8c204ba0f312",
        "write_scope": [
            "team_control_brief_response_contract",
            "team_control_openapi_snapshot",
            "enterprise_ai_erp_web_contract",
            "team_control_enterprise_ai_erp_panels",
            "master_spec_and_control_docs",
            "bas215c_tests_and_evidence",
        ],
        "blocked_on": [],
    }


def _bas183_current_task():
    return {
        "task_id": "BAS-183",
        "state": "in_progress",
        "owner_thread_id": "019fc23a-1ea8-76b0-9688-c11d40eae3e4",
        "write_scope": [
            "governed_media_job_header_event_ledger",
            "durable_image_dispatch_adapter",
            "media_job_reserved_evidence_links",
            "governed_media_job_migration_0097",
            "bas183_core_tests_and_evidence",
        ],
        "blocked_on": [],
    }
...
def test_bas183_claims_media_lane_with_migration_lease_only():
    registry = _registry()
    lanes = {lane["id"]: lane for lane in registry["lanes"]}
    media = lanes["J"]

    assert media["current_task"] == _bas183_current_task()
    assert media["next_task_id"] is None
    assert registry["shared_write_leases"]["alembic_migration"] == "BAS-183"
    assert registry["shared_write_leases"]["api_aggregation_root"] is None
    assert registry["shared_write_leases"]["openapi_snapshot"] == "BAS-215C"
    assert registry["shared_write_leases"]["master_spec"] == "BAS-215C"
    plan = PLAN_PATH.read_text(encoding="utf-8")
    bas183_row = next(
    ...
    assert bas183_row.endswith("| IN_PROGRESS |")
...
def test_data_cov_002_release_no_longer_owns_coverage_or_migration_leases():
    ...
    assert coverage["current_task"] == _bas215c_current_task()
...
def test_bas212_release_record_and_bas210_release_are_preserved():
    ...
    assert registry["shared_write_leases"] == {
        "alembic_migration": "BAS-183",
        "api_aggregation_root": None,
        "master_spec": "BAS-215C",
        "openapi_snapshot": "BAS-215C",
    }
    plan = PLAN_PATH.read_text(encoding="utf-8")
    bas212_row = next(
...
def test_bas215b_release_hands_lane_m_to_bas215c_delivery():
    registry = _registry()
    lanes = {lane["id"]: lane for lane in registry["lanes"]}
    coverage = lanes["M"]
    engineering = lanes["C"]
    ...
    assert bas215c_row.endswith("| IN_PROGRESS |")
