...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"nested": [{"api-key": "sk_live_ABCDEFGHIJKLMNOPQRSTUV"}]},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"consent_owner": "approved prose sk_live_ABCDEFGHIJKLMNOPQRSTUV123456"},
            {"contract_id": "consent-v1"},
        ),
        (
            {"status": "authorized"},
...
def test_deeply_encoded_provider_secret_has_zero_persistence():
    evidence, authority = services()
    unsafe = "sk_live_ABCDEFGHIJKLMNOPQRSTUV123456"
    for _ in range(8):
        unsafe = "".join(
...
def test_review_rationale_secret_is_rejected_and_never_persisted():
    evidence, authority = services()
    submission = _submit_consent(authority)
    raw_secret = "sk_live_ABCDEFGHIJKLMNOPQRSTUV123456"
    with pytest.raises(ValueError, match="Credential material"):
        authority.review(
            principal=principal("reviewer-a", "reviewer"),
            entity_scope=ENTITY_SCOPE,
