...
def main() -> None:
    ...
    path = (
        "/v1/growth-experiments/workspace?store_ref=ozon-primary"
        "&as_of=2026-07-30T01:00:00%2B00:00"
    )
    with httpx.Client(
        base_url="http://127.0.0.1:8000", timeout=30
    ) as client:
        anonymous = client.get(path)
        first = client.get(path, headers={"X-KJDS-API-Key": allowed})
        second = client.get(path, headers={"X-KJDS-API-Key": allowed})
        forbidden = (
            client.get(path, headers={"X-KJDS-API-Key": denied})
            if denied
            else client.get(
                "/v1/growth-experiments/workspace"
    ...
    result = {
        ...
        "upstream_read": payload.get("control_envelope", {}).get(
            "upstream_read"
        ),
        "legacy_marketplace_growth_used": payload.get(
            "control_envelope", {}
        ).get("legacy_marketplace_growth_used"),
    ...
    if any(result.get(key) != value for key, value in expected.items()):
        raise RuntimeError(f"BAS-155 runtime boundary drifted: {result}")
