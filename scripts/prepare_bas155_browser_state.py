...
def main() -> None:
    ...
    with httpx.Client(
        ...
        if login_page.status_code != 200 or response.status_code != 303:
            raise RuntimeError("BAS-155 browser authentication failed")
        cookies = []
    ...
    if not cookies:
        raise RuntimeError("BAS-155 browser login returned no session cookies")
    target = root / ".runtime" / "bas155-browser-auth.json"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(
        json.dumps({"cookies": cookies, "origins": []}),
        encoding="utf-8",
    )
    print("BAS-155 temporary browser state prepared without credential output")
