...
def main() -> None:
    ...
    origin = str(
        values.get("KJDS_WEB_PUBLIC_ORIGIN")
    ...
    with httpx.Client(
        ...
        if page.status_code != 200 or response.status_code != 303:
            raise RuntimeError(
                "BAS-157 browser authentication failed"
            )
        cookies = []
    ...
    if not cookies:
        raise RuntimeError(
            "BAS-157 browser login returned no cookies"
        )
    target = root / ".runtime" / "bas157-browser-auth.json"
    ...
    print(
        "BAS-157 temporary browser state prepared "
        "without credential output"
    )
